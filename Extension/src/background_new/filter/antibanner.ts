...
export const antiBannerService = (() => {
    ...
    async function onFiltersLoadedFromStorage(rulesFilterMap) {
        ...
        function isEmptyRulesFilterMap(rulesFilterMap) {
            ...
            return !hasFilterRules(rulesFilterMap, AntiBannerFiltersId.USER_FILTER_ID);
        }
        ...
        const requestFilterInitialized = function () {
             ...
             */
            if (isEmptyRulesFilterMap(rulesFilterMap) /* TODO: && !stealthService.hasFilterRules() */) {
                return;
            }
        ...
        const startTSUrlFilterEngine = async () => {
            ...
            for (const filterId in rulesFilterMap) {
                ...
                // TODO: support trusted
                // const isTrustedFilter = subscriptions.isTrustedFilter(filterId);

                if (id === AntiBannerFiltersId.USER_FILTER_ID) {
                    userrules = rulesFilterMap[filterId];
                    continue;
                }

                if (id === AntiBannerFiltersId.ALLOWLIST_FILTER_ID) {
